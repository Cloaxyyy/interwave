...
impl SymphoniaDecoder {
    pub(crate) fn new(
        ...
        match SymphoniaDecoder::init(mss, extension) {
            Err(e) => match e {
                ...
                Error::SeekError(_) => {
                    // fMP4 (YouTube adaptive streams) can return SeekError during
                    // gapless-metadata probing. Don't panic — surface as decode error.
    ...
    fn init(
        ...
        let format_opts: FormatOptions = FormatOptions {
            // Disable gapless: YouTube fMP4 adaptive streams trigger a seek during
            // gapless-metadata probing that fails and causes a panic on some tracks.
        ...
        // Save what we need from `stream` before the borrow ends (NLL).
        let fallback_sample_rate = stream.codec_params.sample_rate;
        let fallback_channels    = stream.codec_params.channels;
        // `stream` borrow ends here; `probed.format` is now available for &mut.

        // Try to read and decode the first packet to get a real SignalSpec.
        //
        // For fragmented MP4 (YouTube adaptive streams) the IsoMP4 format reader
        // can return SeekError on the very first next_packet() call because it
        // tries to seek to the fragment data offset.  In that case we fall back to
        // codec-parameter SignalSpec and an empty buffer.  Iterator::next() checks
        //   `current_frame_offset >= buffer.len()`  →  0 >= 0 → true
        // on the very first call, so it immediately reads the next packet; at that
        // point the reader has advanced past the problematic probe state and
        // next_packet() succeeds normally.
        let mut decode_errors: usize = 0;
        let initial_decoded = 'read: loop {
            match probed.format.next_packet() {
                Err(Error::SeekError(_)) => break 'read None,  // fMP4 — use fallback
                Err(e) => return Err(e),
                Ok(pkt) => match decoder.decode(&pkt) {
                    Ok(decoded) => break 'read Some(decoded),
        ...
        let (spec, buffer) = match initial_decoded {
            Some(decoded) => {
                let spec = decoded.spec().to_owned();
                let buf  = SymphoniaDecoder::get_buffer(decoded, &spec);
                (spec, buf)
            }
            None => {
                // Fallback: build SignalSpec from codec parameters; empty buffer.
                let sr = fallback_sample_rate.unwrap_or(44100);
                let ch = fallback_channels
                    .ok_or_else(|| Error::Unsupported("no channel layout in codec params"))?;
                let spec = SignalSpec::new(sr, ch);
                let buf  = SampleBuffer::new(0, spec);
                (spec, buf)
            }
        };
